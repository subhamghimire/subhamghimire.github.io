  ...
  return (
    <SectionShell id="about">
      ...
      <div className="grid gap-5 lg:grid-cols-12 lg:gap-6">
        <article className="rounded-2xl border border-white/10 bg-[#111821]/70 p-5 shadow-xl shadow-black/15 sm:p-6 lg:col-span-5">
          ...
          <ul className="mt-5 flex flex-wrap gap-2">
            {softwareSkills.map((skill) => (
              <li
                key={skill.skillName}
                className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#0b1016]/80 px-3 py-1.5 text-sm text-slate-200 shadow-sm shadow-black/20"
              >
                {skill.skillName}
              </li>
            ))}
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:col-span-7">
          {whatIDo.map((item) => {
            ...
            return (
              <article
                key={item.title}
                className="rounded-2xl border border-white/10 bg-[#111821]/65 p-5 shadow-xl shadow-black/15 hover:border-white/20 hover:bg-[#141d28]/85"
              >
